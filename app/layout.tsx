...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en" className={brand.variable}>
      <body className={`${brand.className} min-h-screen bg-white text-neutral-900 antialiased`}>
        <header className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-white/70 border-b border-neutral-200">
          <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
            ...
            <Link href="/contact" className="rounded-full border px-3 py-1 text-xs hover:bg-neutral-50">Contact</Link>
          </div>
        </header>
